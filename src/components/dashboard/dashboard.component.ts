...
import { Joy, JoyCategory, JoyGroup, JoyGroupMember } from '../../types/joy.interface';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, TranslatePipe, AddExpenseDialogComponent],
  template: `
    <div class="p-4 md:p-8 max-w-6xl mx-auto">
      
      <div *ngIf="!joyId" class="flex flex-col items-center justify-center h-[50vh] rounded-2xl border border-slate-200 bg-white p-6 md:p-8 text-center dark:border-slate-800 dark:bg-slate-900 shadow-sm">
        <span class="material-symbols-outlined text-5xl text-slate-300 dark:text-slate-600 mb-4">auto_awesome</span>
        <h3 class="text-2xl font-bold text-slate-900 dark:text-white">{{ 'dashboard.selectJoy' | translate }}</h3>
        <p class="mt-2 text-slate-500 dark:text-slate-400 max-w-sm mx-auto">{{ 'dashboard.selectJoyDesc' | translate }}</p>
      </div>
      ...
      <ng-container *ngIf="joyId && selectedJoy as joy">
        ...
        <header class="md:hidden pb-4">
          <div class="flex items-center justify-between mb-2">
            <button type="button" (click)="onBackToJoysClick()" class="p-2 -ml-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
          ...
          <h1 class="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">{{ joy.joyName }}</h1>
          <p class="text-slate-500 dark:text-slate-400 text-sm mt-1 font-medium flex items-center gap-1">
            <span class="material-symbols-outlined text-sm">calendar_today</span>
            {{ joy.date || ('dashboard.noDate' | translate) }} • {{ joy.category }}
          </p>
        </header>
        ...
        <header class="hidden md:block mb-8">
          <div class="relative h-64 w-full rounded-2xl overflow-hidden shadow-sm group">
            ...
            <img src="https://images.unsplash.com/photo-1525625293386-3f8f99389edd?ixlib=rb-4.0.3&auto=format&fit=crop&w=1000&q=80" alt="Cover" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
            <div class="absolute bottom-0 left-0 p-8 z-20 w-full flex justify-between items-end">
              <div>
                <h2 class="text-white text-4xl font-black tracking-tight mb-2">{{ joy.joyName }}</h2>
                <div class="flex items-center gap-2 text-white/90">
                  <span class="material-symbols-outlined text-sm">calendar_today</span>
                  <span class="text-sm font-medium">{{ joy.date || ('dashboard.noDate' | translate) }} • {{ joy.category }}</span>
                </div>
              </div>
              <div class="flex items-center gap-2">
                <button type="button" (click)="onBackToJoysClick()" class="bg-white/20 backdrop-blur-md hover:bg-white/30 text-white border border-white/30 px-5 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2">
                ...
                <button type="button" (click)="onNewGroupClick()" class="bg-primary hover:bg-primary/90 text-white border border-primary/70 px-5 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2 shadow-lg shadow-primary/30">
        ...
        <section class="grid grid-cols-2 gap-4 md:gap-6 mb-8">
          <div class="bg-primary/5 md:bg-white dark:bg-primary/10 md:dark:bg-slate-900 p-4 md:p-6 rounded-xl md:rounded-2xl border border-primary/10 md:border-slate-200 dark:border-primary/20 md:dark:border-slate-800 shadow-sm flex flex-col justify-between">
            <p class="text-slate-500 dark:text-slate-400 text-xs md:text-sm font-semibold uppercase tracking-wider mb-1 md:mb-2">{{ 'dashboard.totalGroupSpend' | translate }}</p>
            <p class="text-xl md:text-3xl font-black text-slate-900 dark:text-white">{{ formatAmount(totalSpent) }}</p>
          </div>
          
          <div class="bg-slate-50 md:bg-white dark:bg-slate-800 md:dark:bg-slate-900 p-4 md:p-6 rounded-xl md:rounded-2xl border border-slate-100 md:border-slate-200 dark:border-slate-700 md:dark:border-slate-800 shadow-sm flex flex-col justify-between">
            <p class="text-slate-500 dark:text-slate-400 text-xs md:text-sm font-semibold uppercase tracking-wider mb-1 md:mb-2">{{ 'dashboard.totalYouOwe' | translate }}</p>
            <p class="text-xl md:text-3xl font-black text-rose-500">{{ formatAmount(totalYouOwe) }}</p> 
          </div>
        </section>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-8">
            <div class="flex items-center justify-between mb-4 md:mb-6">
              <h3 class="text-lg md:text-xl font-bold text-slate-900 dark:text-white">{{ 'dashboard.yourGroups' | translate }}</h3>
            </div>

            <div *ngIf="!groupCards.length" class="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center dark:border-slate-700 dark:bg-slate-900">
              <span class="material-symbols-outlined text-5xl text-slate-300 dark:text-slate-600">group</span>
              <h3 class="mt-3 text-xl font-bold text-slate-900 dark:text-white">{{ 'dashboard.blankTitle' | translate }}</h3>
              <p class="mt-1 text-slate-500 dark:text-slate-400">{{ 'dashboard.blankDesc' | translate }}</p>
            </div>
            ...
            <div class="space-y-4" *ngIf="groupCards.length > 0">
              ...
              >
                <div class="flex items-center gap-4 min-w-0 flex-1">
                  <div class="w-12 h-12 rounded-full overflow-hidden flex-shrink-0 bg-primary/10 text-primary flex items-center justify-center">
                    <img *ngIf="group.photo" [src]="group.photo" [alt]="group.name" class="w-full h-full object-cover" />
                    <span *ngIf="!group.photo" class="material-symbols-outlined">receipt_long</span>
                  </div>
                  <div class="min-w-0">
                    <h4 class="font-bold text-slate-900 dark:text-white truncate">{{ group.name }}</h4>
                    <p class="text-xs text-slate-500 dark:text-slate-400 font-medium mt-0.5 truncate">
                      {{ group.members.length }} {{ 'dashboard.members' | translate }} 
                    </p>
                  </div>
                </div>
                <div class="ml-3 w-50 shrink-0 text-right sm:w-36">
                  <p class="text-sm font-bold text-slate-900 dark:text-white sm:text-base">{{ formatAmount(group.totalSpent) }}</p>
                </div>
                ...
                <div class="ml-3 flex shrink-0 items-center gap-2">
                  ...
                  >
                    ...
                    <span class="hidden sm:inline">{{ 'groupDetail.addExpense' | translate }}</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          <div class="lg:col-span-4 pb-20 lg:pb-0">
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
              <div class="p-4 md:p-6 border-b border-slate-100 dark:border-slate-800/50">
                <h3 class="text-lg font-bold text-slate-900 dark:text-white">{{ 'dashboard.yourSplitBills' | translate }}</h3>
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">{{ 'dashboard.memberSplit' | translate }}</p>
              </div>
              
              <div class="grid grid-cols-2 gap-3 p-4 md:grid-cols-1 md:gap-0 md:p-0">
                <div *ngFor="let item of splitBillSummaries" class="rounded-xl border border-slate-100 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-950/50 md:rounded-none md:border-0 md:bg-transparent md:p-4 md:border-b md:border-slate-50 md:dark:border-slate-800/50">
                  <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm" [ngClass]="getAvatarColorClasses(item.name)">
                  ...
                  <p class="mt-3 text-sm font-bold text-slate-900 dark:text-white md:mt-0 md:text-right">{{ formatAmount(item.amount) }}</p>
                </div>
                
                <div *ngIf="!splitBillSummaries.length" class="col-span-2 p-8 text-center md:col-span-1">
                  <span class="material-symbols-outlined text-3xl text-slate-300 dark:text-slate-600">pie_chart</span>
                  <p class="text-sm text-slate-500 dark:text-slate-400 mt-2">No split details available yet.</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <ng-template #joyConfigDialog>
        <div class="space-y-4">
          <div class="flex flex-col gap-2">
            <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">{{ 'joys.joyName' | translate }}</label>
            <input
          ...
          <div class="flex flex-col gap-2">
            <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">{{ 'joys.category' | translate }}</label>
            <select
              [(ngModel)]="joyConfigForm.category"
              [disabled]="isSavingJoyConfig"
              class="w-full rounded-xl border-transparent bg-slate-50 px-4 py-3 outline-none transition-all focus:border-primary focus:ring-2 focus:ring-primary/20 dark:bg-slate-800 dark:text-slate-100"
            >
              <option *ngFor="let cat of joyCategories" [value]="cat">{{ cat }}</option>
            </select>
          </div>
          ...
          <p *ngIf="joyConfigErrorMessage" class="text-sm text-red-600 dark:text-red-300">{{ joyConfigErrorMessage }}</p>
        </div>
      </ng-template>
    </div>
  `
...
export class DashboardComponent implements OnChanges, OnDestroy {
  ...
  @ViewChild('quickAddExpenseDialog') quickAddExpenseDialog!: AddExpenseDialogComponent;
  @ViewChild('joyConfigDialog', { static: true }) joyConfigDialog!: TemplateRef<unknown>;
  ...
  getStatusLabel(type: 'owed' | 'owe' | 'settled'): string {
    if (type === 'owed') return this.translationService.t('dashboard.status.owed');
    if (type === 'owe') return this.translationService.t('dashboard.status.owe');
    return this.translationService.t('dashboard.status.settled');
  }

  getBalanceText(group: JoyGroup): string {
    if (group.balanceType === 'owed') return this.currencyService.formatSignedAmount(Math.abs(group.yourBalance));
    if (group.balanceType === 'owe') return this.currencyService.formatSignedAmount(-Math.abs(group.yourBalance));
  ...
  get splitBillSummaries(): SplitBillSummary[] {
    ...
    for (const group of this.groupCards) {
      ...
      for (const member of members) {
        ...
        const current = totals.get(key) ?? { key, name: member.name || this.translationService.t('groupDetail.unknown'), amount: 0 };
        const amount = this.getMemberSplitAmountValue(group, member.shareAmount, memberCount);
  ...
  private getMemberSplitAmountValue(group: JoyGroup, memberShareAmount: number | undefined, memberCount: number): number {
    if (typeof memberShareAmount === 'number' && Number.isFinite(memberShareAmount) && memberShareAmount >= 0) {
  ...
  private getJoyConfigActions(): CommonDialogAction[] {
    return [
      ...
      {
        label: () => this.translationService.t(this.isSavingJoyConfig ? 'dashboard.savingJoyConfig' : 'dashboard.saveJoyConfig'),
        icon: () => (this.isSavingJoyConfig ? 'progress_activity' : 'save'),
        kind: 'primary',
        grow: true,
        disabled: () => this.isSavingJoyConfig || !this.isJoyConfigFormValid() || !this.joyId,
        handler: () => { void this.saveJoyConfig(); }
      }
    ];
  }
  ...
  private async saveJoyConfig(): Promise<void> {
    ...
    try {
      await this.joyService.updateJoy(this.joyId, {
        ...
        date: this.formatJoyDateRange(this.joyConfigForm.startDate, this.joyConfigForm.endDate)
      });
      this.commonDialogService.close();
    } catch (error) {
      console.error('Unable to update joy config.', error);
      this.joyConfigErrorMessage = 'Unable to save joy settings. Please try again.';
    } finally {
      this.isSavingJoyConfig = false;
    }
  }
  ...
  private parseJoyDateRange(value: string): { startDate: string; endDate: string } {
