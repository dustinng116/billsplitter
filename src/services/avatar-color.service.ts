...
export class AvatarColorService {
  private readonly palette: AvatarColorPair[] = [
  ...
  getInitialAvatarClasses(seed: string): string {
    const normalizedSeed = seed?.trim().toLowerCase() || 'unknown';
    const hash = this.hashCode(normalizedSeed);
    const pair = this.palette[hash % this.palette.length];
